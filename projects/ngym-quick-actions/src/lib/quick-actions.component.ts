...
export class QuickActionsComponent implements OnChanges {
  ...
  @Input({ required: false }) scrollEventsObservable?: Observable<void>;
  ...
  prepareActions(): void {
    this.iconActions = this.actions.filter(
      (action) => action instanceof ViewIconQuickAction
    );
    this.dropDownActions = this.actions.filter(
      (action) => action instanceof ViewDropDownQuickAction
    );
  ...
  prepareDropdownActions(): void {
    ...
    this.dropDownActions.forEach((action) => {
      this.dropDownItems.push({
        ...
        label: action.label,
        action: action,
        linkClasses: !action.isActionAllowed
          ? this.textStdClasses.disabled
          : '',
        isLoadingClasses: action.checkIsLoading
          ? this.textStdClasses.disabled
          : '',
      });
    });
  ...
  performDropdownAction(action: ViewIconQuickAction | ViewDropDownQuickAction) {
    if (
      ...
      (!action.checkIsLoading || (action.checkIsLoading && !this.isLoading))
    )
